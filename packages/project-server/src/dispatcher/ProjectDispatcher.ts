...
import { DatabasePools, ERROR_CODE, HttpHandler, I18nReader, IHttpResponseError, MapOfString } from "aitianyu-server-base";
import { PROJECT_ERROR_CODE } from "../common/Errors";
import { IQuery } from "../common/Types";

const _download_excludes: string[] = ["node-modules"];

function ___processProjectPackMemberName(member: string): string {
...
function ___processProjectMemberItem(source: MapOfString): any {
...
export class ProjectDispatcher {
    ...
    private projectDBMap: MapOfString;

    public constructor(dbPool: DatabasePools, i18n: I18nReader) {
    ...
    private async __getAllProjects(messageList: IHttpResponseError[]): Promise<any> {
        return new Promise((resolve) => {
            try {
                this.databasePool.execute(
                    "aitianyu_base",
    ...
    private async _projectMacrodefDispatcher(query: IQuery, messageList: IHttpResponseError[]): Promise<any> {
        return new Promise((resolve) => {
            ...
            oPromise.finally(() => {
                ...
                const getTotalPromise = new Promise<number>((done) => {
                ...
                getTotalPromise.then((totalRows: number) => {
    ...
    private async __projectAPIBrowser_Basic(
        projectName: string,
        language: string,
    ...
    private async __projectAPIBrowser_Namespace(
        projectName: string,
        pack: string,
        language: string,
        messageList: IHttpResponseError[],
    ...
    private async __projectAPIBrowser_Member(
        projectName: string,
        pack: string,
        member: string,
        language: string,
        ...
        return new Promise((resolve) => {
            ...
            try {
                this.___projectAPIBrowser_GetMemberDefinition(dbName, decodePach, decodeMember, messageList).then(
                    (result: any) => {
    ...
    private async ___projectAPIBrowser_GetMemberDefinition(
        dbName: string,
        pack: string,
        member: string,
        messageList: IHttpResponseError[],
    ): Promise<{ def: any[]; type: string }> {
        const sql =
    ...
    private async ___projectAPIBrowser_FillMemberItems(
        projectName: string,
        dbName: string,
        pack: string,
        member: string,
        language: string,
        messageList: IHttpResponseError[],
    ): Promise<{ constructor: any[]; property: any[]; operator: any[]; method: any[] }> {
        const sql =
